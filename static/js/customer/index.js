        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                $.each(data,function (i,p) {
                    if(p.m_gmessage !=null){
                        _html = ` <div class="m_g">
                            <span>回复：</span>
                            <span class="m_g">${p.m_gmessage}</span>
                            <span class="gtime">${p.m_gdate}</span>
                        </div>`;
                    }
                    else{
                        _html = '';
                    }
                    html +='<div class="m_n"><span>@'+p.m_name+'</span> <span class="m_p">'
                        +p.m_message+'</span><span class="time">'
                        +p.m_cdate+'</span>'
                        +_html+'</div>';
    ...
    $('#save').click(function(e){
        ...
        if($('#m_message').val()){
            $.ajax({
                ...
                data:{'u_name':uname,'m_message':$('#m_message').val()},
                success:function (data) {
                    getMesssage();
                    $('#m_message').val('');
                }
...
$('.T-classfy .Tclassfy-lf').on('click','dd',function (e) {
    e.preventDefault();
    window.location.href = '../../customer/account/productsTime.html?ptime='+$(this).data('id');
});
